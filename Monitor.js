//created on 2018.1.4
//creator: Mark Zhang
//email: m-super8@foxmail.com

//---------------------------------------------------variables-------------------------------------------------------
var diagram_frequence=60;//s
var capture_frequence=60*30;//s
...
$(document.body).html("<div id='capture'>"+$(document.body).html()+"</div><div id='chartContainer' style='height: 600px; width:100%;'></div>");
...
//arrays
var currentval=new Array([datalen]);
for(var i=0;i<datalen;i++){
 currentval[i] = 0;
}

var people=new Array([datalen]);
$("#DataTables_Table_0 tbody tr").each(function(){
    $(this).children("td").each(function(index, Element){
      if(index==0)
      {
          seq=parseInt($(this).html());
      }
      else if(index==1)
      {
          //名字
          people[seq]=$(this).html();
      }
      else if (index==4)
      {
          //当前票数
          var c=parseInt($(this).children("span").html());
          currentval[seq]=c;
      }
    });
});
var difference=new Array([datalen]);

//capture interval
var int=self.setInterval("capture()",capture_frequence*1000);
function capture()
  {
      var time = new Date().format("yyyy年MM月dd日 hh:mm:ss");
      var timeid=new Date().getSeconds();
      html2canvas(document.getElementById("capture")).then(canvas => {
          ...
          $(this).children("td").each(function(index, Element){
              ...
              else if (index==4)
              {
                  //当前票数
                  var c=parseInt($(this).children("span").html());
                  difference[seq]=c-currentval[seq];
                  currentval[seq]=c;
              }
          });
      });

          var t="<div><table border='8' width='100%'><tr><th colspan='2' align='center'><h1>"+time+"</h1></th></tr><tr><th>选项</th><th>"+capture_frequence+"秒内增长票数</th></tr>";
          for(var i=1;i<currentval.length;i++){
              t+="<tr>";
              t+="<td>"+people[i]+"</td>";
              t+="<td>"+difference[i]+"</td>";
              t+="</tr>";
          }
          t+="</table></div>";
          $(document.body).append(t); 

          $(document.body).append('<div id='+timeid+'></div>');
            document.getElementById(timeid).appendChild(canvas);
      });
  }
capture();
...
//arrays
var currentval2=new Array([datalen]);
for(var i=0;i<datalen;i++){
 currentval2[i] = 0;
}

var people2=new Array([datalen]);
$("#DataTables_Table_0 tbody tr").each(function(){
    $(this).children("td").each(function(index, Element){
      if(index==0)
      {
...
var chart;
var data_array=[];
jQuery.getScript("https://canvasjs.com/assets/script/canvasjs.min.js", function(data, status, jqxhr) {
    for(var i=1;i<currentval2.length;i++){
        data_array.push({
            type: "spline",
            name: people2[i],
            showInLegend: true,
      ...
      else if (index==4)
      {
		...
		jQuery.getScript("https://canvasjs.com/assets/script/canvasjs.min.js", function(data, status, jqxhr) {
				for(var i=0;i<data_array.length;i++){
					data_array[i]['dataPoints'].push({ x: new Date(), y: currentval2[i+1]});
					/*if (data_array[i]['dataPoints'].length > dataLength) {
            	    data_array[i]['dataPoints'].shift();
            		}*/
				}
        	chart.render();
		});
...
updateChart();
setInterval(function(){ updateChart(); }, diagram_frequence*1000); 
